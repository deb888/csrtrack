...
module.exports = (function() {
	...
	//Create 
	router.post('/:table',authenticate, function(req, res) {
	...
	//Update by ID 
	router.put('/:table/:id',authenticate, function(req, res) {
	...
	//Read by ID 
	router.get('/:table/:id',authenticate, function(req, res) {
		sequelize.query("SHOW KEYS FROM `"+TABLE_PREFIX+req.params.table+"` WHERE Key_name = 'PRIMARY'", { type: sequelize.QueryTypes.SELECT})
		.then(function(keys) {
			var primary_key = keys[0].Column_name;
	...
	//Delete by ID 
	router.delete('/:table/:id',authenticate, function(req, res) {
		sequelize.query("SHOW KEYS FROM `"+TABLE_PREFIX+req.params.table+"` WHERE Key_name = 'PRIMARY'", { type: sequelize.QueryTypes.SELECT})
		.then(function(keys) {
			var primary_key = keys[0].Column_name;
